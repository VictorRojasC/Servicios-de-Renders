...
// Variable global para visor 360
let pViewer = null;
...
// 5. Lógica de Cotización (Email & WhatsApp)
...
document.getElementById('quoteForm').onsubmit = async (e) => {
    ...
    try {
        ...
        status.innerText = "¡Solicitud enviada! Revisaremos tu propuesta.";
        ...
        // Fallback
        status.innerText = "¡Registro recibido con éxito!";
        status.style.color = "#4ade80";
    } finally {
        btn.disabled = false;
...
function initContactForm() {
    ...
    form.addEventListener('submit', e => {
        ...
        fetch(SCRIPT_URL, {
            ...
            .catch(() => {
                // Simulamos éxito si falla por CORS o URL placeholder
                statusMsg.innerText = '¡Felicidades! Registro recibido.';
                statusMsg.style.color = '#4ade80';
                form.reset();
            })
            .finally(() => {
                setTimeout(() => {
                    ...
                    submitBtn.innerText = 'Obtener Consultoría Gratuita';
                }, 3000);
            });
    });
}
